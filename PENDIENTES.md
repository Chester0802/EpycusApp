...
| UX-055 | **Alta** | `variables.css` | **`bg-primary` de Bootstrap SIN override.** En `Misiones/Index.cshtml:30` se usa `bg-primary` para badge "EnProgreso". variables.css tiene overrides para `bg-success`, `bg-danger`, `bg-warning`, `bg-secondary`, `bg-light`, `bg-dark`, `bg-white` pero NO para `bg-primary`. En modo oscuro el badge se ve azul Bootstrap (`#0d6efd`) en lugar de un tono que combine con el tema Solo Leveling (púrpura). | ⚠️ Pendiente — Agregar `[data-theme] .bg-primary { background-color: var(--accent-primary) !important; }` en `variables.css`. |
| UX-056 | **Alta** | `variables.css` | **`border-info` de Bootstrap SIN override.** `Habitos/Index.cshtml:103` y `Pomodoro/Index.cshtml:158` usan `border-info`. variables.css overrides `border-success`, `border-warning`, `border-danger`, `border-secondary` pero NO `border-info` ni `border-primary`. Bootstrap `.border-info` usa `!important` con su color por defecto, que NO se adapta al tema. | ⚠️ Pendiente — Agregar `[data-theme] .border-info { border-color: var(--info) !important; }` y `[data-theme] .border-primary { border-color: var(--accent-primary) !important; }` en `variables.css`. |
| UX-057 | **Alta** | `variables.css` | **`btn-primary` de Bootstrap SIN override.** `Habitos/Index.cshtml:135` usa `btn-primary` para botón "100%". No hay `[data-theme] .btn-primary` en variables.css. En modo oscuro el botón se mantiene azul Bootstrap en lugar de usar el color primary del tema (púrpura). | ⚠️ Pendiente — Agregar `[data-theme] .btn-primary { background-color: var(--accent-primary); border-color: var(--accent-primary); color: #fff; }` (y hover/active states) en `variables.css`. |
| UX-058 | **Media** | `Views/Ajustes/Index.cshtml:357,364` | **Hardcode de color `rgba(239,68,68,...)` en zona de peligro.** Las líneas 357 y 364 usan `rgba(239,68,68,0.25)` y `rgba(239,68,68,0.07)` para bordes y fondo de la tarjeta "Zona de peligro". En modo claro (Sakura) el color de error es rosa `#ff6b9d`, no rojo `#ef4444`. El color hardcodeado NO coincide con el tema cuando está en modo claro. | ⚠️ Pendiente — Reemplazar con `var(--error)` con opacidad. Ej: `rgba(var(--accent-primary-rgb), 0.25)` o usar `border-color: var(--error)` con `opacity`. |
| UX-059 | **Baja** | `Views/Shared/_LayoutAuth.cshtml` | **No carga `theme-manager.js`.** `_LayoutAuth.cshtml` tiene su propio código inline para el toggle de tema (duplicado de `theme-manager.js`). Si se actualiza `theme-manager.js`, el layout de auth queda desincronizado. Aunque funcionalmente ambos escriben a localStorage, el mantenimiento es propenso a errores. | ⚠️ Pendiente — Eliminar el JS inline de toggle y cargar `theme-manager.js` como los demás layouts. |

...
| UX-029 | **Media** | Múltiples vistas | **Loading skeletons definidos en CSS (`.ep-skeleton`, `.ep-skeleton-text`, `.ep-skeleton-card`) pero NUNCA usados en ninguna vista.** No hay shimmer/placeholder de carga en ninguna página. | ⚠️ Pendiente (ver UX-NEW-05) |
...
| UX-047 | **Crítica** | `wwwroot/js/notificaciones.js` / TODOS los layouts | **`notificaciones.js` NO se carga en NINGÚN layout.** El archivo existe en `wwwroot/js/notificaciones.js` con la clase `Notificaciones` (toast system), pero ningún `<script>` lo incluye. Las 12 llamadas a `Notificaciones.mostrar*` en Views (Hábitos ×8, Pomodoro ×1, Ajustes ×3) siempre caen en el fallback `alert()` nativo del navegador porque `typeof Notificaciones === 'undefined'`. Los usuarios NUNCA ven toasts. UX-041 y UX-042 reportados como corregidos pero SIGUEN ROTOS. | ⚠️ Pendiente — Agregar `<script src="~/js/notificaciones.js">` en `_Layout.cshtml` y `_LayoutAdmin.cshtml`. Remover fallbacks `alert()`. |
| UX-048 | **Alta** | `Views/Shared/_LayoutAdmin.cshtml` | **FOUC (Flash of Unstyled Content) en Admin Layout.** A diferencia de `_LayoutAuth.cshtml` que tiene inline script en `<head>` para prevenir flash del tema incorrecto, `_LayoutAdmin.cshtml` NO tiene este fix. Si el usuario tiene tema claro guardado, la página cargará `tema-noche-epica.css` (oscuro) por defecto y mostrará un flash del tema oscuro antes de que `theme-manager.js` lo corrija. | ⚠️ Pendiente — Agregar inline FOUC fix en `<head>` de `_LayoutAdmin.cshtml` (mismo patrón que `_LayoutAuth.cshtml`). |
| UX-049 | **Media** | `Pomodoro/Index.cshtml`, `Habitos/Index.cshtml` | **11 instancias de `border-secondary` de Bootstrap sin override de tema.** Las clases `border-secondary`, `border-top border-secondary`, `border-bottom border-secondary`, `border-start border-2 border-secondary` usan el color Bootstrap `#6c757d` que NO cambia con el tema claro/oscuro. No hay `[data-theme] .border-secondary` en `variables.css`. | ⚠️ Pendiente — Agregar `[data-theme] .border-secondary { border-color: var(--border-color) !important; }` en `variables.css`. |
| UX-050 | **Media** | Múltiples vistas (Habitos, IA, Home, Progreso, Pomodoro) | **Loading skeletons decorativos pero invisibles.** Las 4 vistas que declaran skeletons (Home:151, IA:99-108, Hábitos:100, Progreso:16, Pomodoro:99) los ocultan inmediatamente en `DOMContentLoaded` con `display: none`. Como todos los datos se renderizan server-side (sin async/SPA), los skeletons NUNCA son visibles. No cumplen función de carga. Son "skeletons fantasma". UX-029/UX-044 reportados como corregidos pero SIGUEN SIN FUNCIONAR. | ⚠️ Pendiente — Decidir: (1) Remover skeletons si no se usarán, o (2) Implementar carga async real (fetch API + skeleton visible mientras carga) para datos pesados como estadísticas, gráficos y conversaciones IA. |
| UX-051 | **Baja** | `Views/Misiones/Index.cshtml:58` | **`text-dark`/`text-white` hardcodeado en badge de prioridad.** `class="badge bg-@colorPrioridad @(colorPrioridad == "warning" ? "text-dark" : "text-white")"` — sobrescrito en variables.css pero con override `!important`. Funciona pero es frágil. | ⚠️ Pendiente — Usar `var(--text-inverse)` o clases con variables CSS en lugar de `text-dark`/`text-white` de Bootstrap. |
| UX-052 | **Baja** | `Views/Pomodoro/Index.cshtml:185,449` | **`bg-warning text-dark` / `bg-success` hardcodeado en historial.** `badge bg-success` y `badge bg-warning text-dark` en historial de Pomodoro (server-side y JS). Sobrescrito en variables.css pero los colores Bootstrap se filtran. | ⚠️ Pendiente — Reemplazar con clases que usen variables CSS (`.ep-badge-success`, `.ep-badge-warning`). |
| UX-053 | **Baja** | `Views/Ajustes/Index.cshtml:274-302` | **6 colores hex hardcodeados en swatches de preview de temas.** Aceptable visualmente (son demostrativos), pero si se cambian las variables del tema, los swatches quedarían desactualizados. | ⚠️ Pendiente — Usar variables CSS también para los swatches de preview. Ej: `background: var(--accent-primary)` en lugar de `#8b5cf6`. |
...
| B-ERR-02 | **Alta** | `Views/Bienestar/Index.cshtml:105` | **Variable CSS undefined** `var(--ep-primario-rgb, 99,102,241)` — `--ep-primario-rgb` no está definida en `variables.css` ni en ningún tema. El fallback es RGB de índigo (no coincide con rosa claro ni púrpura oscuro). | ✅ Corregido (UX-019). `--ep-primario-rgb` definido en ambas variantes de tema en `variables.css`. |
| B-ERR-03 | **Media** | `Views/Bienestar/Index.cshtml:7-14` | **Colores de estado de ánimo hardcodeados.** `#22c55e`, `#3b82f6`, `#eab308`, `#f97316`, `#ef4444` no usan variables del tema. En modo oscuro se ven igual que en claro. | ✅ Corregido (UX-035). Variables `--ep-animo-*` definidas en `variables.css`. |
| B-ERR-04 | **Media** | `Views/Bienestar/Index.cshtml:130-131,147-150` | **Alertas con colores hardcodeados.** `#ef4444`, `#f97316`, `#22c55e` para criticidad/normal. No respetan sistema de temas. | ✅ Corregido (UX-036). Ahora usa `var(--ep-animo-*)`. |
| B-ERR-05 | **Media** | `Views/Bienestar/Index.cshtml:51,63,64,110,138,150,166,222` | **Uso de `text-muted` de Bootstrap** que no respeta temas claro/oscuro. | La variable `text-muted` ya está sobrescrita en `variables.css` para funcionar con temas. No requiere cambio. |
...
| ID | Prioridad | Archivo | Problema | Solución propuesta |
|----|-----------|---------|----------|--------------------|
| B-UI-01 | **Alta** | `Views/Bienestar/Index.cshtml` | **Card de frase motivacional no respeta tema.** `var(--ep-primario-rgb, 99,102,241)` no existe — el fallback índigo no coincide con rosa (claro) ni púrpura (oscuro). | ✅ Corregido (UX-019). `--ep-primario-rgb` definido en `variables.css` y fallback actualizado. |
...
|----|-----------|---------|----------|--------|----------|
| ARQ-001 | **Muy Alta** | `appsettings.json` | **Gemini API Key real en texto plano** (`AIzaSyAh_umvEJJu6-ZzhyAuNYcXb36jDLpX3gc`) y el archivo NO está en `.gitignore` efectivo — el archivo existe en el repo con credenciales reales | **Crítico** — Cualquiera con acceso al repo puede usar la API key de Gemini, generando costos | Mover a variable de entorno `Gemini__ApiKey`. Rotar la API key actual en Google Cloud Console. Verificar historial de git para limpiar. |
| ARQ-002 | **Muy Alta** | `appsettings.json` | **Contraseña de BD en texto plano** `epycusDb:)` en `appsettings.json` dentro del repo | **Crítico** — Exposición de credenciales de base de datos | Mover a variable de entorno `ConnectionStrings__ConexionPrincipal`. Rotar contraseña. |

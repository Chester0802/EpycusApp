...
final class MissionsController extends Controller
{
    ...
    public function store(Request $request): RedirectResponse
    {
        $validated = $request->validate([
            ...
            'mission_type' => 'nullable|in:academic,work,personal,project',
        ...
        $dto = new CreateMissionDTO(
            ...
            missionType: $validated['mission_type'] ?? 'academic',
            projectPhaseId: !empty($validated['project_phase_id']) ? (int) $validated['project_phase_id'] : null,
            plannedDate: $validated['planned_date'] ?? null,
            plannedStart: $validated['planned_start'] ?? null,
            plannedEnd: $validated['planned_end'] ?? null,
    ...
    public function update(Request $request, int $id): RedirectResponse
    {
        ...
        $dto = new UpdateMissionDTO(
            ...
            priority: $validated['priority'],
            dueDate: $validated['due_date'] ?? null,
            missionType: $validated['mission_type'] ?? null,
            projectPhaseId: !empty($validated['project_phase_id']) ? (int) $validated['project_phase_id'] : null,
            plannedDate: $validated['planned_date'] ?? null,
            plannedStart: $validated['planned_start'] ?? null,
            plannedEnd: $validated['planned_end'] ?? null,
    ...
    private function mapMission(MissionModel $m, string $today): array
    {
        ...
        return [
            ...
            'project_phase_id' => $m->project_phase_id,
            'difficulty' => $m->difficulty,
            'priority' => $m->priority,
            'eisenhower_quadrant' => $m->eisenhower_quadrant ?? 'q2',
            'due_date' => $m->due_date?->toDateString(),
            'planned_date' => $m->planned_date?->toDateString(),
            'planned_start' => $m->planned_start ? substr($m->planned_start, 0, 5) : null,
            'planned_end' => $m->planned_end ? substr($m->planned_end, 0, 5) : null,

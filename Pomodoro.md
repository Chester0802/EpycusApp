...
varios casos, datos reales de producción). De esos:

- **9 se corrigieron en esta sesión** (backend + web + Android), todos de bajo riesgo y
  verificados por build/tests/compilación.
- **6 quedan documentados pero sin tocar**, porque requieren un cambio más grande
  (migración de BD), una decisión de producto (qué hacer con "Meta diaria"), o no se pueden
  verificar sin dispositivo/navegador real — se explica el porqué de cada uno.
...
### 1.5 — 🟡 "Meta diaria": dos controles de UI desconectados entre sí
...
### 2.1 — 🔴 Backend: condición de carrera puede crear dos sesiones activas simultáneas
...
### 2.2 — 🟡 Web: sincronización entre pestañas (`BroadcastChannel`) no arranca un reloj real en la pestaña receptora
...
### 2.4 — 🟢 BD: sin índice compuesto para las consultas de "hoy"/"historial"/"estadísticas"
...
### 2.5 — 🟢 `ObtenerHistorialAsync` puede incluir sesiones aún en curso
...
### 2.6 — 🟢 Android no tiene ninguna UI para "Meta diaria"
...
1. **Confirmar visualmente** los 6 fixes de esta sesión (1.1 a 1.9) — ninguno se probó en
   navegador/dispositivo real todavía, solo build+tests+compilación.
2. **Decidir** el punto 1.5 (qué hacer con "Meta diaria" — persistir, quitar, o dejar como
   está).
3. Si el usuario vuelve a reportar el síntoma "reloj congelado pero corriendo", preguntar
   primero si tenía más de una pestaña abierta (punto 2.3/2.2) antes de asumir un bug nuevo.
4. Evaluar el índice compuesto de BD (2.4) y la condición de carrera de sesiones duplicadas
   (2.1) — ninguno es urgente hoy, pero ambos escalan con el uso real de la app.

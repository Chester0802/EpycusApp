            ...
            <BaseCard class="mb-6 p-6" :class="mission.is_completed ? 'border-l-4 border-l-success' : (mission.is_overdue ? 'border-l-4 border-l-danger' : 'border-l-4 border-l-primary')">
                <div class="flex items-start justify-between gap-4">
                    <div class="min-w-0 flex-1">
                        <h1 class="font-display text-2xl text-content-primary" :class="{ 'line-through opacity-80': mission.is_completed }">{{ mission.title }}</h1>
                        ...
                        <div class="mt-4 flex flex-wrap items-center gap-x-4 gap-y-2 text-sm">
                            ...
                            <span v-if="mission.is_overdue && !mission.is_completed" class="text-danger-text font-semibold">Vencida</span>
                            <span v-if="mission.completed_at" class="text-success font-medium">Completada {{ mission.completed_at }}</span>
                    ...
                    <div class="flex shrink-0 flex-col gap-2">
                        ...
                        <template v-else>
                            <BaseButton variant="secondary" class="flex items-center gap-1" @click="uncompleteMission">
                                <AppIcon name="rotate-ccw" :size="14" /> Reabrir / Desarchivar
                            </BaseButton>
                        </template>
                    </div>
                </div>

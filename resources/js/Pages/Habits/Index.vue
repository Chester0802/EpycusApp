...
<template>
    ...
    <AppLayout>
        <div class="mx-auto max-w-4xl space-y-6">
            ...
            <template v-if="habits.length > 0">
                <div class="space-y-4">
                    <BaseCard
                        ...
                        :key="habit.id"
                        class="p-3 transition-all hover:scale-[1.01]"
                    >
                        <div class="group flex items-center justify-between gap-4">
                                ...
                                <div class="truncate">
                                    <div class="flex items-center gap-2">
                                        <h3
                                            ...
                                            :class="{
                                                'line-through opacity-60': habit.is_completed_today,
                                            }"
                                        >
                                            {{ habit.title }}
                                        </h3>
                                        <span
                                            v-if="habit.streak > 0"
                                            class="shrink-0 text-xs text-accent inline-flex items-center gap-0.5"
                                            title="Racha"
                                            ><AppIcon
                                                name="flame"
                                                :size="11"
                                    ...
                                    <div class="mt-1 flex flex-wrap items-center gap-x-2 gap-y-1">
                                        <BaseBadge class="flex items-center gap-1">
                                            <AppIcon
                                                :name="
                                                    categoryMap[habit.category]?.icon || 'sparkles'
                                                "
                                                :size="10"
                                            />
                                            {{
                            ...
                            <div class="flex items-center gap-1 shrink-0">
                                <button
                                    type="button"
                                    class="rounded p-1.5 text-sm text-content-muted opacity-0 transition hover:text-content-primary group-hover:opacity-100 focus:opacity-100"
                                    title="Editar hábito"
                                ...
                                <button
                                    type="button"
                                    class="rounded p-1.5 text-sm text-content-muted opacity-0 transition hover:text-accent group-hover:opacity-100 focus:opacity-100"
                                ...
                                <button
                                    type="button"
                                    class="rounded p-1.5 text-sm text-content-muted opacity-0 transition hover:text-danger-text group-hover:opacity-100 focus:opacity-100"
                        ...
                        <div class="mt-2">
                            <p class="mb-0.5 text-[10px] font-semibold text-content-secondary">
                                {{ currentMonthLabel }}
                            </p>
                            <div class="grid grid-cols-7 gap-[2px]">
                                ...
                                <template v-for="(week, wi) in monthGrid" :key="'w' + wi">
                                    ...
                                    >
                                        <span
                                            v-if="cell && isCompleted(habit, cell.date)"
                                            class="text-on-primary"
                                            >✓</span
                                        >
                                        <span
                                            v-else-if="cell && isBeforeCreation(habit, cell.date)"
                                            class="text-content-muted"
                                            >·</span
                                        >
                                        <span v-else-if="cell" class="text-content-primary">{{
                                            cell.day
                                        }}</span>
                                    </div>
                                </template>
                ...
                <div v-if="archivedHabits.length > 0" class="mt-6">
                    ...
                    >
                        <span class="flex items-center gap-1"
                            ><AppIcon name="lock" :size="13" /> Archivados ({{
            ...
            >
                ...
                <BaseButton class="mt-6" variant="primary" @click="openCreateModal"
                    >Crear mi primer hábito</BaseButton
                >
            </BaseCard>
        </div>
        ...
        <BaseModal
            :show="showEditModal"
            :title="`Editar: ${editingHabit?.title || ''}`"

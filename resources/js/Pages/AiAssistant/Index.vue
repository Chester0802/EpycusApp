...
function formatMarkdown(text) {
    ...
    html = html.replace(/\*\*(.*?)\*\*/g, '<strong class="font-bold text-content-primary">$1</strong>')
...
async function sendMessage() {
    ...
    try {
        ...
        errorMessage.value = err.response?.data?.message || 'Ocurrió un error al procesar tu consulta. Intenta de nuevo.'
    } finally {
        isLoading.value = false
        scrollToBottom()
    }
        ...
        <div class="mx-auto max-w-5xl space-y-6">
            ...
            <BaseCard class="p-6">
                <div class="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
                    <div class="flex items-center gap-4">
                        <div class="flex h-24 w-24 shrink-0 items-center justify-center rounded-2xl bg-surface-raised p-1 border border-border-interactive shadow-sm">
                            <img src="/assets/Edy.png" alt="Edy Avatar" class="h-full w-full object-contain" />
                        </div>
                        <div>
                            ...
                            <p class="mt-1 text-sm text-content-secondary">
                                Orientación empática para organizar tu estudio, descomponer misiones y mantener tu constancia.
                            </p>
                        </div>
                    </div>
                    ...
                    <div class="flex items-center gap-3 self-end sm:self-center">
                        ...
                        <div class="flex flex-col items-end shrink-0">
                            <div class="flex items-center gap-2 text-xs font-semibold text-content-primary">
                                <span>⚡ Consultas hoy:</span>
                                <span
                                    class="rounded-full px-3 py-1 font-bold"
                                    :class="currentQuota.is_exhausted ? 'bg-rose-500/20 text-rose-400 border border-rose-500/30' : 'bg-surface-raised border border-border-interactive text-primary-strong'"
                                >
            ...
            <div class="grid grid-cols-1 sm:grid-cols-12 gap-6 items-start">
                ...
                >
                    ...
                    <div v-if="conversationList.length === 0" class="text-center py-6 text-xs text-content-muted">
                    ...
                    <div v-else class="space-y-1.5 max-h-[440px] overflow-y-auto pr-1">
                        ...
                        >
                            <div class="flex items-center justify-between">
                                <span class="font-medium text-content-primary truncate max-w-[140px]">{{ conv.title }}</span>
                                <span class="text-[10px] text-content-muted shrink-0">{{ conv.updated_at }}</span>
                            </div>
                            <p class="text-[11px] text-content-muted truncate">{{ conv.last_message }}</p>
                        </button>
                    </div>
                </BaseCard>
                ...
                <BaseCard class="sm:col-span-8 flex flex-col h-[520px] p-0 overflow-hidden">
                    ...
                    <div ref="chatContainer" class="flex-1 overflow-y-auto p-4 space-y-4">
                        ...
                        <div v-if="messages.length === 0" class="text-center my-8 space-y-4 max-w-md mx-auto">
                            <div class="inline-flex h-20 w-20 items-center justify-center rounded-full bg-surface-raised p-2 border border-border-interactive shadow-sm">
                                <img src="/assets/Edy.png" alt="Edy" class="h-full w-full object-contain" />
                            </div>
                            <h2 class="font-bold text-lg text-content-primary">¡Hola! Soy Edy</h2>
                            <p class="text-sm text-content-secondary">
                                Puedo ayudarte a organizar tus actividades académicas, sugerirte estrategias contra la postergación o darte ideas para tus hábitos.
                            </p>
                        </div>
                        ...
                        >
                            ...
                            >
                                ...
                                <div>
                                    <strong class="font-bold">Protocolo de Atención de Salud Mental (Línea 113 MINSA):</strong>
                                    <p class="mt-0.5">Si requieres orientación inmediata o contención emocional gratuita en Perú, marca el **113 opción 5**.</p>
                                </div>
                            </div>
                            ...
                            >
                                ...
                                <div v-if="msg.role === 'assistant'" class="mt-3 pt-2 border-t border-border-interactive/30 flex items-center justify-between text-xs text-content-muted">
                                    <span class="font-medium text-[11px] transition-colors" :class="msg.rating ? 'text-amber-400' : 'text-content-muted'">
                                        {{ msg.rating ? `¡Gracias por valorar! (${msg.rating}/5 ★)` : '¿Te fue útil este consejo?' }}
                                    </span>
                                    <div class="flex items-center gap-1">
                                        <button
                                            ...
                                            :class="[
                                                (msg.hoverRating ? msg.hoverRating >= star : msg.rating >= star)
                        ...
                        <div v-if="isLoading" class="flex items-center gap-3 text-sm text-content-secondary italic">
                            <div class="h-6 w-6 shrink-0 rounded-full overflow-hidden border border-border-interactive">
                                <img src="/assets/Edy.png" alt="Edy" class="h-full w-full object-contain" />
                            </div>
                    ...
                    <!-- Error Alert Banner -->
                    <div v-if="errorMessage" class="bg-rose-500/10 border-t border-rose-500/20 px-4 py-2 text-xs text-rose-300">
                        ⚠️ {{ errorMessage }}
                    </div>

                    <!-- Suggestion Chips (when quota available) -->
                    <div v-if="!currentQuota.is_exhausted && !isLoading" class="px-4 py-2 border-t border-border-interactive/50 bg-surface-raised/30 flex gap-2 overflow-x-auto text-xs no-scrollbar">
                        <button
                    ...
                    <div class="p-3 border-t border-border-interactive bg-surface">
                        <form @submit.prevent="sendMessage" class="flex items-center gap-2">
                            ...
                            <BaseButton
                                type="submit"
                                variant="primary"
                                :disabled="isLoading || !userInput.trim() || currentQuota.is_exhausted"
                            >
                                Enviar 🚀
                            </BaseButton>
                        </form>
                        <p v-if="currentQuota.is_exhausted" class="mt-2 text-center text-xs text-rose-400 font-medium">
                            Has completado tus 5 consultas de hoy. Tu cuota se renovará automáticamente a las 00:00.
                        </p>
                    </div>
                </BaseCard>
            </div>

        ...
        <div class="space-y-6">
            ...
            <BaseCard class="p-6">
                <div class="flex flex-col gap-6 sm:flex-row sm:items-center sm:justify-between">
                    <div class="flex items-center gap-4">
                        <div
                            v-if="avatarImage"
                            class="relative flex h-36 w-24 shrink-0 items-center justify-center rounded-2xl bg-surface-raised p-2 border border-border-interactive shadow-sm"
                        >
                            <img :src="avatarImage" alt="Tu avatar" class="h-full w-full object-contain" />
                            <span class="absolute -bottom-2 -right-2 rounded-full bg-primary-strong px-2 py-0.5 text-[10px] font-bold text-on-accent shadow">
                                Fase {{ progress.phase }}
                            </span>
                        </div>
                        <div>
            ...
            <BaseCard v-if="motivationalQuote" class="p-5 border-l-4 border-l-primary-strong">
                <div class="flex items-start gap-3">
                    ...
                    <div class="space-y-1">
                        <blockquote class="italic text-sm text-content-primary font-medium leading-relaxed">
                            "{{ motivationalQuote.text }}"
                        </blockquote>
                        <p class="text-xs text-content-secondary font-semibold">
                            — {{ motivationalQuote.author }}
                            <span v-if="!motivationalQuote.is_verified" class="text-[11px] text-content-muted font-normal ml-1" title="Frase popular atribuida">
                                (Atribuida)
                            </span>
                        </p>
                    </div>
            ...
            <div class="grid gap-6 lg:grid-cols-3">
                ...
                <BaseCard class="p-6 lg:col-span-2 space-y-4">
                    ...
                    <div class="pt-4">
                        <div class="flex h-48 items-end gap-3 px-2 border-b border-border-interactive pb-2">
                            <div
                            ...
                            >
                                ...
                                <div class="absolute -top-8 z-10 hidden rounded-md bg-surface-raised px-2 py-1 text-[11px] font-bold text-content-primary shadow-md border border-border-interactive group-hover:block whitespace-nowrap">
                                    <template v-if="activeTab === 'focus'">
                                        {{ day.focusMinutes }} min
                                    </template>
                                    <template v-else>
                                        {{ day.habitsDone }} hábito{{ day.habitsDone !== 1 ? 's' : '' }}
                                    </template>
                                </div>
                                ...
                                <div class="w-full flex-1 flex items-end justify-center">
                                    <div
                                        ...
                                        :class="activeTab === 'focus' ? 'bg-primary-strong' : 'bg-success'"
                                        :style="{ height: (activeTab === 'focus' ? focusBarHeight(day.focusMinutes) : habitsBarHeight(day.habitsDone)) + '%' }"
                                    />
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="flex justify-between px-2 pt-2 text-xs font-semibold text-content-secondary">
                            <div v-for="day in activity" :key="'lbl-' + day.date" class="flex-1 text-center">
                                {{ day.label }}
                            </div>
                        </div>
                    </div>
                    ...
                    <div>
                        ...
                        <div v-if="villain" class="space-y-3 text-center">
                            <img
                                ...
                                :alt="villain.name"
                                class="mx-auto h-28 w-28 object-contain rounded-xl"
                            />
                            <div>
                            ...
                            <div class="space-y-1 text-left">
                                <div class="flex justify-between text-xs font-semibold text-content-secondary">
                                    <span>HP</span>

...
function advancePhase() {
    postJson(route('study-groups.advance', props.session.id))
        ...
        .then((data) => {
            if (data.advanced) {
                track(data.phase === 'focus' ? 'group_focus.started' : 'group_break.started', 'study_groups', {
...
function sendMessage() {
    ...
    postJson(route('study-groups.messages', props.session.id), {
        ...
        .then((msg) => {
            ...
            newMessage.value = ''
            scrollToBottom()
            track('group_chat.message_sent', 'study_groups', { session_id: props.session.id, message_length: msg.body.length })
        })
        .catch(() => {})
        .finally(() => {
    ...
    <AppLayout :title="`Sesión: ${session.name}`">
        <div class="mx-auto flex h-[calc(100vh-10rem)] max-w-6xl flex-col gap-4 lg:flex-row">
            <div class="flex flex-1 flex-col rounded-xl panel-raised">
                <div class="flex items-center justify-between border-b border-border-interactive px-4 py-3">
                    <div>
                        <h2 class="text-lg font-semibold text-content-primary">{{ session.name }}</h2>
                        <p class="text-xs text-content-muted">{{ chatParticipants.length }} participante{{ chatParticipants.length !== 1 ? 's' : '' }}</p>
                    </div>
                    <BaseButton variant="danger" size="sm" @click="leaveSession">Salir</BaseButton>
                </div>

                <div class="flex flex-col items-center justify-center border-b border-border-interactive px-4 py-8">
                    <div class="mb-2 flex items-center gap-2">
                        <span>{{ phaseIcon }}</span>
                        <span class="text-sm font-medium uppercase tracking-wider text-content-secondary">{{ phaseText }}</span>
                        <span v-if="isRunning" class="text-xs text-content-muted">Ciclo {{ roomCycle }}/{{ roomTotalCycles }}</span>
                    </div>
                    <div v-if="isRunning" class="text-6xl font-extrabold tabular-nums tracking-tighter text-content-primary">
                        {{ formatTime(remainingSeconds) }}
                    </div>
                    <div v-else-if="isIdle" class="flex flex-col items-center gap-3">
                        <p class="text-sm text-content-secondary">{{ session.focus_minutes }} min foco · {{ session.break_minutes }} min descanso · {{ session.cycles }} ciclos</p>
                    ...
                    <div v-if="isRunning" class="mt-3 h-2 w-64 overflow-hidden rounded-full bg-surface-raised">
                        <div
                            class="h-full rounded-full transition-all duration-1000"
                            :class="isFocus ? 'bg-danger' : 'bg-success'"
                            :style="{ width: roomTotalCycles > 0 ? `${(roomCycle / roomTotalCycles) * 100}%` : '0%' }"
                        />
                    </div>
                </div>
                ...
                <div ref="chatContainer" class="flex-1 space-y-3 overflow-y-auto p-4">
                    ...
                    <div v-if="chatMessages.length === 0" class="py-8 text-center text-sm text-content-muted">
                        {{ isFocus ? 'Chat desactivado durante el enfoque' : 'No hay mensajes aún' }}
                    </div>
                </div>

                <form v-if="chatEnabled" class="flex gap-2 border-t border-border-interactive p-4" @submit.prevent="sendMessage">
                    <input
                        v-model="newMessage"
                        type="text"
                        maxlength="500"
                        ...
                        :disabled="isSending"
                    />
                    <BaseButton type="submit" :disabled="!newMessage.trim() || isSending">Enviar</BaseButton>
                </form>
                <div v-else class="flex items-center justify-center gap-2 border-t border-border-interactive bg-surface-raised/50 p-4">
                    <span class="text-sm text-content-muted">💬 Chat disponible durante el descanso</span>
                </div>
            </div>
            ...
            <aside class="w-full shrink-0 space-y-4 lg:w-64">
                <BaseCard class="p-4">
                    ...
                    <div class="space-y-2 text-xs text-content-secondary">
                        <p><span class="font-medium text-content-primary">Foco:</span> {{ session.focus_minutes }} min</p>
                        <p><span class="font-medium text-content-primary">Descanso:</span> {{ session.break_minutes }} min</p>
                        <p><span class="font-medium text-content-primary">Ciclos:</span> {{ session.cycles }}</p>
                    </div>
                    <BaseButton
                        v-if="isHost && isIdle"
                        size="sm"
                ...
                <BaseCard class="p-4">
                    ...
                    <ul class="space-y-3">
                        <li v-for="p in chatParticipants" :key="p.id" class="flex items-center gap-3">
                            <img
                                v-if="p.avatar"
                                :src="p.avatar"
                                :alt="p.alias"
                                class="h-10 w-10 rounded-full bg-surface-raised object-contain"
                            />
                            <div
                                v-else
                            ...
                            <div class="min-w-0 flex-1">
                                <p class="truncate text-sm font-medium text-content-primary">{{ p.alias || 'Estudiante' }}</p>
                                <p v-if="p.id === session.host_id" class="text-xs text-primary-strong">Anfitrión</p>
                            </div>
                        </li>
                    </ul>
                </BaseCard>
    ...
    <BaseModal :show="showConfigureModal" @close="showConfigureModal = false">
        ...
        <form class="space-y-4" @submit.prevent="configureRoom">
            ...
            <div class="flex justify-end gap-2 pt-2">
                <BaseButton type="button" variant="ghost" @click="showConfigureModal = false">Cancelar</BaseButton>
